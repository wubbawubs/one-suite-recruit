...
import roles, { roleClusters, getRolesByCluster } from "@/data/roles";

export default function RolesHub() {
  return (
    <div className="min-h-screen">
      <PageMeta
        title="Functies: Executive Search per Rol | One Time Recruit"
        description="Overzicht van alle leiderschapsfuncties waarvoor One Time Recruit executive search uitvoert. Van CEO tot Operations Director."
        locale="nl_NL"
        jsonLd={{ "@context": "https://schema.org", "@type": "CollectionPage", name: "Functies — Executive Search per Rol", provider: { "@type": "Organization", name: "One Time Recruit" } }}
      />
      <Header />
      <div className="container">
        <Breadcrumbs items={[{ label: "Home", href: "/nl" }, { label: "Diensten", href: "/nl/diensten" }, { label: "Functies" }]} />
      </div>
        ...
        <section className="relative overflow-hidden bg-primary py-20 md:py-28 lg:py-36">
          ...
          <div className="container relative z-10">
            <ScrollReveal className="mx-auto max-w-3xl text-center">
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-accent">Alle functies</span>
              <h1 className="mt-4 font-display text-3xl font-bold leading-[1.08] text-primary-foreground md:text-5xl lg:text-[3.5rem]">
                Executive search per <span className="text-accent">functie</span>
              </h1>
              <p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-primary-foreground/60">
                Van CEO tot Operations Director. Rolprofielen, interviewvragen, succes-KPI's en salarisbenchmarks.
              </p>
              <div className="mt-10">
        ...
        <section className="border-b border-border/50 bg-muted/40 py-10">
          <div className="container">
            <div className="grid grid-cols-2 gap-8 md:grid-cols-4 text-center">
              {[
                { value: `${roles.length}+`, label: "Functies" },
          ...
          <div className="container">
            <div className="space-y-16">
              {roleClusters.map((cluster) => {
                const clusterRoles = getRolesByCluster(cluster.id);
                if (clusterRoles.length === 0) return null;
                const Icon = clusterRoles[0].icon;
                return (
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <RoleIcon className="h-4 w-4 text-muted-foreground group-hover:text-accent transition-colors" />
                              <div>
                                <span className="text-sm font-semibold text-card-foreground">{role.shortTitle}</span>
        ...
        <section className="relative overflow-hidden bg-accent py-20 md:py-24">
          ...
          <div className="container relative z-10 text-center">
            <ScrollReveal variant="scale-in">
              <h2 className="font-display text-3xl font-bold text-accent-foreground">Uw functie niet gevonden?</h2>
              <p className="mx-auto mt-5 max-w-lg text-lg text-accent-foreground/80">
                Neem contact op — wij plaatsen leiders voor elke senior leiderschapspositie.
              </p>
              <div className="mt-10">
